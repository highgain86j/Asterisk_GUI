...
if ($auth->isAuthorized())
{
	...
	if(isset($_POST['import']))
	{
		...
		{
			$importSuccess = 'No File Selected, choose a file.';
		}
		//AstLinux// end of missing file fix.
	}
			...
			else
			{
				...
				for($i=0; $i<sizeof($result); $i++)
				{
					...
					if($typeAffinity=="INTEGER" || $typeAffinity=="REAL" || $typeAffinity=="NUMERIC")
						...
						//AstLinux//
						echo "<input type='text' size='60' id='".htmlencode($field)."_search' name='".htmlencode($field)."'/>";
	...
	//- Row actions
		...
		case "row_view":
			...
			if(sizeof($arr)>0)
			{
				...
				if($target_table_type == 'view')
				{
					//AstLinux// Remove WiKi link
					echo sprintf($lang['readonly_tbl'], htmlencode($target_table)); 
					echo "<br/><br/>";	
				}
			...
			for($j=0; $j<$num; $j++)
			{
				...
				for($i=0; $i<sizeof($result); $i++)
				{
					...
					if($typeAffinity=="INTEGER" || $typeAffinity=="REAL" || $typeAffinity=="NUMERIC")
						...
						//AstLinux//
						echo "<input type='text' size='60' id='row_".$j."_field_".$i."_value' name='".$j.":".$field_html."' value='".$dflt_value."' onblur='changeIgnore(this, \"row_".$j."_ignore\");' onclick='notNull(\"row_".$j."_field_".$i."_null\");'/>";
					...
					for($j=0; $j<sizeof($pks); $j++)
					{
						...
						for($i=0; $i<sizeof($result); $i++)
						{
							...
							if($typeAffinity=="INTEGER" || $typeAffinity=="REAL" || $typeAffinity=="NUMERIC")
								...
								//AstLinux//
								echo "<input type='text' size='60' name='".htmlencode($field)."[]' value='".htmlencode($value)."' onblur='changeIgnore(this, \"".$j."\", \"".htmlencode($pks[$j]).":".htmlencode($field)."_null\")' />";
...
//AstLinux// Remove Resource stuff until end of file, get .js and .css from repo.
